...
function islocalurl(url) {
    try {
        ...
        return (
            ...
            hostname === ''                     // file:// URLs have empty hostname
        );
    } catch (e) {
        // url is not a valid URL (maybe a relative path), consider it local
        return true;
    }
}
...
function getjsonf(url, callback) {
    ...
    if (url.endsWith('.json') || url.startsWith('https')) {
        request.overrideMimeType("application/json");
        if (islocal && document.title !== 'gallery') {
...
function indexsidenav(navdir = "") {
    ...
    if (data)
        Object.entries(data).forEach((entry) => {
            ...
            if (document.title === value.section && value.private === false) {
                text += value.href;
                text += "<b>" + value.name.toLowerCase() + "</b></a>";
            }
            //if (value.name.indexOf(".io") < 0 && value.private === false) {
            if (document.title !== 'playlist' && value.section !== 'playlist' && value.private === false) {
                text += value.href;
                text += "<b>" + value.name.toLowerCase() + "</b></a>";
            };
...
// todo evaluate passing year in this manner
...
// after scrolls down 20px from the top of the document show button
